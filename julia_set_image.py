...
FILENAME = f'pictures/stills1/still1.jpg'
...
def save_julia_set_image(file_name: str, expression: str, real_range_min: float = -2.31, real_range_max: float = 2.31, imag_range_min: float = -1.3, imag_range_max: float = 1.3, image_width=1920, image_height=1080, label_image=False, a=0):
    ...
    img = plotting_helper(calculate_julia_set, a, 200, real_range_min, real_range_max, imag_range_min, imag_range_max, image_width, image_height, label_image, expression)
    background = Image.new("RGB", img.size, (255, 255, 255))
    background.paste(img, mask=img.split()[3]) # 3 is the alpha channel
    
    background.save(fp=file_name, format='JPEG')


def plotting_helper(expression: callable, a, iteration_count: int = 100, real_range_min: float = -2.31, real_range_max: float = 2.31, imag_range_min: float = -1.3, imag_range_max: float = 1.3, image_width=1920, image_height=1080, label_image=False, function_string=''):
    ...
    img = Image.fromarray(np.uint8(matplotlib.cm.gnuplot(normalized) * 255))
    if(label_image):
...
def main():
    ...
    strfunc = 'x**4 + x**3 / (x - 1) + x**2 / (x**3 + 4 * x**2 + 5) + 0.377767 * sin(2) + 0.368646 * 1j * sin(2) - 0.368646 * 1j + 0.377767'
    save_julia_set_image(FILENAME, strfunc, REAL_RANGE_MIN, REAL_RANGE_MAX, IMAG_RANGE_MIN, IMAG_RANGE_MAX, IMAGE_WIDTH, IMAGE_HEIGHT, True)
